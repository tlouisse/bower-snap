...
const lock = require('bower-locker/bower-locker-lock.js');
const unlock = require('bower-locker/bower-locker-unlock.js');
// const validate = require('bower-locker/bower-locker-validate.js');
...
async function comparePath() {
    try {
        const comparisonFileStr = fs.readFileSync(`${program.comparePath}`, `utf-8`, {output:false});
        validate(program.verbose, comparisonFileStr);
    } catch (e) {
        console.error(e);
    }
...
['snap', 'compare', 'comparePath', 'lock', 'unlock'].forEach(c => program[c] && bowerSnap[c](program.verbose));
